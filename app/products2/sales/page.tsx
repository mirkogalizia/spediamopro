...
import {
  Box, Paper, Typography, TableContainer, Table, TableHead, TableBody,
  TableRow, TableCell, Chip, Alert, Skeleton, Stack, Divider,
} from "@mui/material";
...
type TypeGroup = { tipologia: string; variants: VariantData[]; };
...
function parseVariante(v: string | null | undefined): { size: string; color: string } {
  const parts = (v ?? "").split("/").map(p => p.trim()).filter(Boolean);
  if (parts.length === 0) return { size: "Sconosciuta", color: "Sconosciuto" };
  if (parts.length === 1) return TAGLIE_SET.has(normalize(parts[0])) ? { size: parts[0], color: "Sconosciuto" } : { size: "Sconosciuta", color: parts[0] };
  const [a, b] = parts;
  const aIs = TAGLIE_SET.has(normalize(a)); const bIs = TAGLIE_SET.has(normalize(b));
  if (aIs && !bIs) return { size: a, color: b || "Sconosciuto" };
  if (!aIs && bIs) return { size: b, color: a || "Sconosciuto" };
...
function aggregateByColorSize(variants: VariantData[]) {
  ...
  for (const v of variants) {
    ...
    const taglia = size || "Sconosciuta"; const colore = color || "Sconosciuto";
    ...
    if (current === 0 && incoming > 0) grouped[colore][taglia].stock = incoming;
    // se vuoi sempre l'ultimo: grouped[colore][taglia].stock = incoming;
  }
  return grouped;
}
...
export default function StockForecastByColorAndSize() {
  ...
  async function fetchData() {
    ...
    setError('');
    try {
      const res = await fetch(`/api/products/sales?ts=${Date.now()}`, { cache: 'no-store' });
  ...
  return (
    <Box sx={{ minHeight: "100vh", bgcolor: "background.default", py: 6, px: 1, maxWidth: "1100px", mx: "auto" }}>
      ...
      <Typography variant="h3" fontWeight={800} sx={{
        mb: 6, background: "linear-gradient(90deg, #005bea 0%, #00c9a7 80%)",
        WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent",
      }}>
        Previsionale Magazzino Blanks
      </Typography>

      {error && <Alert severity="error" sx={{ mb: 3, fontWeight: 600 }}>{error}</Alert>}
      ...
      {!loading && data.map(group => {
        ...
        return (
          <Paper key={group.tipologia} elevation={4} sx={{ mb: 6, borderRadius: 4, p: 4, bgcolor: "#fff", boxShadow: "0 4px 32px 0 #b3e0ff22" }}>
                    ...
                    <TableBody>
                      {Object.entries(taglieObj)
                        ...
                        .map(([taglia, vals]) => {
                          ...
                          const daily = vendutoTot / periodDays;
                          const daysRemaining = daily > 0 ? Math.floor(stockSingolo / daily) : "∞";
                          const purchaseSuggestion = daily > 0 ? Math.max(0, Math.ceil(daily * 30 - stockSingolo)) : 0;

                          return (
                            <TableRow key={taglia} sx={{ bgcolor: typeof daysRemaining === "number" && daysRemaining <= 7 ? "#fff2f2" : "#fff" }}>
                              <TableCell sx={{ fontWeight: 600, fontSize: 17, textTransform: "uppercase" }}>{taglia}</TableCell>
                              <TableCell align="center"><Chip label={vendutoTot} color="primary" variant="outlined" size="medium" sx={{ fontWeight: 700, fontSize: 15 }} /></TableCell>
                              <TableCell align="center">
                                <Chip
                                  label={stockSingolo}
                                  color={stockSingolo === 0 ? "error" : stockSingolo <= 5 ? "warning" : "success"}
                                  variant="filled"
                                  size="medium"
                                  sx={{ fontWeight: 700, fontSize: 15 }}
                                />
                              </TableCell>
                              <TableCell align="center" sx={{ fontWeight: 700, color: typeof daysRemaining === "number" && daysRemaining <= 7 ? "#d93025" : "#006400", fontSize: 16 }}>
                                {daysRemaining}
                              </TableCell>
                              <TableCell align="center" sx={{ fontWeight: 700, fontSize: 16 }}>
                                {purchaseSuggestion > 0 ? <Chip label={`+${purchaseSuggestion}`} color="info" variant="outlined" size="small" sx={{ fontWeight: 700 }} /> : "-"}
                              </TableCell>
                            </TableRow>
                          );
                        })}
