...
export default function StockForecastByColorAndSize() {
  ...
  function onStockChange(tipologia: string, colore: string, taglia: string, value: string) {
    const key = `${normalize(tipologia)}|${normalize(colore)}|${normalize(taglia)}`;
    const num = parseInt(value, 10);
    setStockInput(prev => ({ ...prev, [key]: isNaN(num) ? 0 : num }));
  }
