...
function fmtCurrency(v: number, currency = 'EUR') {
  try {
    return new Intl.NumberFormat('it-IT', { style: 'currency', currency }).format(v || 0);
  } catch {
    return `${v?.toFixed?.(2) ?? v} ${currency}`;
  }
}
...
export default function KPIPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/kpi-spedizioni?${qs}`, { cache: 'no-store' });
  ...
  // Aggregazione per grafico: costo medio giornaliero
  const chartData = useMemo(() => {
    ...
    // genera range completo per continuità
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `kpi-spedizioni_${from}_${to}.csv`;
    a.click();
        ...
        <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Dashboard professionale con dati letti da SpediamoPro in tempo reale.
            </p>
          </div>
