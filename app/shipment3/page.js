...
export default function Page() {
  ...
  const handleSimula = async (e) => {
    ...
    try {
      const res = await fetch("/api/spediamo3?step=simula", {
        method: "POST",
  ...
  const handleCreaECompletaEPaga = async (idSim) => {
    ...
    setErrore(null);
    try {
      const resC = await fetch(`/api/spediamo3?step=create&id=${idSim}&shopifyOrderId=${selectedOrderId}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const resU = await fetch(`/api/spediamo3?step=update&id=${spedizione.id}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          nome: form.nome,
      ...
      const resP = await fetch(`/api/spediamo3?step=pay&id=${spedizione.id}`, { method: "POST" });
      let dataP;
      try {
      ...
      const resDetails = await fetch(`/api/spediamo3?step=details&id=${spedizione.id}`, { method: "POST" });
      let details = {};
      if (resDetails.ok) {
  ...
  const handlePrintLdv = async (idSpedizione) => {
    ...
    setErrore(null);
    try {
      const res = await fetch(`/api/spediamo3?step=ldv&id=${idSpedizione}`, { method: "POST" });
      if (!res.ok) throw await res.json();
  ...
  return (
    <div style={containerStyle}>
      ...
      <div style={cardStyle}>
        <h2 style={headerStyle}>Gestione Spedizioni Shopify - Biscotti Sinceri</h2>
