...
function getTrackingLabel(spedizione) {
  ...
  return (
    spedizione.tracking_number_corriere ||
    spedizione.tracking_number ||
    spedizione.segnacollo ||
    ""
  );
}
      ...
      >
        ...
        >
          ...
          <div>
            ...
            >
              {isEvadi
                ? "Shopify aggiornato con il tracking"
                : "Spedizione confermata e pagata"}
            </div>
          </div>
        </div>
        <div
...
export default function Page() {
      ...
      const aggiornate = await Promise.all(
        spedizioniCreate.map(async (el) => {
          ...
          const res = await fetch(
            `/api/spediamo2?step=details&id=${el.spedizione.id}`,
            { method: "POST" }
          );
          if (res.ok) {
            const d = await res.json();
            return {
              ...el,
              spedizione: { ...el.spedizione, ...d.spedizione },
            };
          }
          return el;
        })
  ...
  const handleLoadOrders = async () => {
    ...
    try {
      if (!dateFrom || !dateTo)
        throw new Error("Specificare data inizio e fine.");
      if (dateFrom > dateTo)
        throw new Error("Data inizio successiva alla data fine.");
  ...
  const handleSearchOrder = (e) => {
    ...
    if (!found) {
      setErrore(`Ordine "${orderQuery}" non trovato.`);
      return;
    }
  ...
  const handleQuota = async (e) => {
    e.preventDefault();
    if (!selectedOrder) {
      setErrore("Seleziona prima un ordine.");
      return;
    }
    setLoading(true);
    setErrore(null);
      ...
      const res = await fetch("/api/spediamo2?step=accept", {
        ...
        body: JSON.stringify({
          ...
          quotation: {
            ...
            accessoryServicePrice:
              quotation.priceBreakdown?.accessoryServicePrice,
      ...
      setModal({
        ...
        dati: [
          ...
          { label: "ID Spedizione", value: `#${spedizione.id}` },
          {
            label: "Corriere",
            value:
              spedizione.courierService?.courier?.toUpperCase() || "—",
          },
          {
            label: "Servizio",
            value: spedizione.courierService?.code || "—",
          },
          {
            label: "Tracking",
            value:
              getTrackingLabel(spedizione) || "in elaborazione…",
            highlight: true,
          },
          {
            label: "Consegna prev.",
            value:
              spedizione.expectedDeliveryDate?.split(" ")[0] || "—",
          },
          {
            label: "Formato etich.",
            value:
              spedizione.labelOption?.originalExtension?.toUpperCase() ||
              "—",
          },
          {
            label: "Costo",
            value: `€ ${(quotation.totalPrice / 100).toFixed(2)}`,
            highlight: true,
          },
        ],
      });
  ...
  const handleDownloadLabel = async (idSpedizione) => {
    ...
    try {
      const res = await fetch(
        `/api/spediamo2?step=labels&id=${idSpedizione}`,
        {
          method: "POST",
        }
      );
      ...
      const bytes = Uint8Array.from(atob(label.b64), (c) =>
        c.charCodeAt(0)
      );
      ...
      a.href = url;
      a.download = label.filename || `etichetta_${idSpedizione}.pdf`;
  ...
  const handleEvadiSpedizione = async (el) => {
    ...
    try {
      ...
      const foundOrder =
        orders.find((o) => o.id === el.shopifyOrder?.id) ||
        el.shopifyOrder;
      if (!foundOrder)
        throw new Error("Ordine Shopify non trovato. Ricarica gli ordini.");
      const res = await fetch("/api/shopify2/fulfill-order", {
        ...
        body: JSON.stringify({
          ...
          carrierName:
            corriere.toLowerCase().includes("sda") ||
            corriere.toLowerCase().includes("poste")
              ? "Poste Italiane"
              : corriere || "Altro",
        }),
      });
      ...
      setSpedizioniCreate((prev) =>
        prev.map((s) =>
          s.spedizione.id === el.spedizione.id
            ? { ...s, fulfilled: true }
            : s
        )
      );
  ...
  const handleCancella = (idSpedizione) => {
    setConfirm({
      ...
      onConfirm: async () => {
        ...
        try {
          const res = await fetch(
            `/api/spediamo2?step=cancel&id=${idSpedizione}`,
            { method: "POST" }
          );
          if (!res.ok) throw await res.json();
          setSpedizioniCreate((prev) =>
  ...
  const handleSvuotaLista = () => {
    setConfirm({
      messaggio:
        "Svuotare la lista locale delle spedizioni? I dati non potranno essere recuperati.",
      onConfirm: () => {
        setConfirm(null);
        setSpedizioniCreate([]);
  ...
  if (!userChecked)
    return (
      <div style={{ padding: 40, textAlign: "center" }}>Caricamento...</div>
    );

  // ─── Wallet color + importo reale ───────────────────────────
  const rawBalance = wallet?.balance ?? null;
        ...
        >
          <div style={{ fontSize: 13 }}>
            ...
            <span style={{ color: "#94a3b8" }}>
              {" "}
              — {mittente.address}, {mittente.city}
            </span>
          </div>
          <HoverButton
            style={{
          ...
          >
            {[
              ...
              <FormField key={key} label={label}>
                <input
                  value={mittente[key] || ""}
                  onChange={(e) =>
                    setMittente((m) => ({ ...m, [key]: e.target.value }))
                  }
                  style={{
                    padding: "7px 10px",
            ...
            >
              ...
              <HoverButton
                ...
                onClick={() => {
                  setMittente(DEFAULT_MITTENTE);
                  setShowMittente(false);
                }}
              >
                ↩ Default
              </HoverButton>
              ...
              >
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      📅 {q.expectedDeliveryDate} · 🚚 ritiro{" "}
                      {q.firstAvailablePickupDate}
                    </div>
                  </div>
                </div>
                <div
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      <div>
                        ...
                        >
                          {el.spedizione.consignee?.name || ""} ·{" "}
                          {corriere.toUpperCase()} · #{el.spedizione.id}
                        </div>
                      </div>
                    </div>
                    <div
                    ...
                    >
                      {new Date(el.createdAt).toLocaleDateString("it-IT", {
                        day: "2-digit",
                        month: "short",
                        hour: "2-digit",
                        minute: "2-digit",
                  ...
                  >
                    <HoverButton
                      ...
                      onClick={() =>
                        handleDownloadLabel(el.spedizione.id)
                      }
                      disabled={loading}
                    >
                      ⬇ Etichetta
                    ...
                    {!el.fulfilled && (
                      <HoverButton
                        ...
                        title={
                          !tracking
                            ? "Tracking non ancora disponibile"
                            : ""
                        }
                      >
                        📬 Evadi su Shopify
                    ...
                    <HoverButton
                      ...
                      onClick={() =>
                        handleCancella(el.spedizione.id)
                      }
                      disabled={loading}
                    >
                      ✕ Cancella
